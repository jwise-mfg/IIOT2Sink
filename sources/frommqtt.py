...
class mqttsource():
    ...
    def connect(self, sinkadapters):
        ...
        clientid = self.config['source']['mqtt']['clientid'] + "-" + str(round(time.time() * 1000))
        print(f"Connecting paho-mqtt version: {paho.mqtt.__version__} to {self.config['source']['mqtt']['broker']}:{self.config['source']['mqtt']['port']} with client id {clientid}")
        ...
        subscribed_topics = []
        for sub in self.mqtt_subscriptions:
            if sub["topic"] not in subscribed_topics:
                print("-Subscribing to:", sub["topic"])
                client.subscribe(sub["topic"])
                subscribed_topics.append(sub["topic"])
        #client.subscribe("#")
        print(f"Awaiting messages for {len(subscribed_topics)} topics...")
